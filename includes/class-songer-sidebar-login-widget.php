...
/**
 * Songer_Sidebar_Login_Widget class.
 * @extends WP_Widget
 */
class Songer_Sidebar_Login_Widget extends WP_Widget {
	private $instance = '';
	private $user     = null;
    ...
    public function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'description' => __( 'Displays a login area in the sidebar.', 'songer-sidebar-login' ) );
		/* Create the widget. */
		parent::__construct( 'wp_sidebarlogin', __( 'Sidebar Login', 'songer-sidebar-login' ), $widget_ops );
    }
    /**
    ...
    public function define_options() {
		...
		$this->options       = array(
			'logged_out_title' => array(
				'label'           => __( 'Logged-out title', 'songer-sidebar-login' ),
				'default'         => __( 'Login', 'songer-sidebar-login' ),
				'type'            => 'text'
			),
			'logged_out_links'  => array(
				'label'           => __( 'Links', 'songer-sidebar-login' ) . ' (<code>' . __( 'Text | HREF', 'songer-sidebar-login' ) . '</code>)',
				'default'         => '',
				'type'            => 'textarea'
			),
			'show_lost_password_link'  => array(
				'label'           => __( 'Show lost password link', 'songer-sidebar-login' ),
				'default'         => 1,
				'type'            => 'checkbox'
			),
			'show_register_link'  => array(
				'label'           => __( 'Show register link', 'songer-sidebar-login' ),
				'default'         => 1,
				'description'     => sprintf( __( '<a href="%s">Anyone can register</a> must be enabled.', 'songer-sidebar-login' ), admin_url('options-general.php') ),
				'type'            => 'checkbox'
			),
			'show_rememberme'  => array(
				'label'           => __( 'Show "Remember me" checkbox', 'songer-sidebar-login' ),
				'default'         => 1,
				'type'            => 'checkbox'
			),
			'login_redirect_url'  => array(
				'label'           => __( 'Login Redirect URL', 'songer-sidebar-login' ),
				'default'         => '',
				'type'            => 'text',
				'placeholder'     => __( 'Current page URL', 'songer-sidebar-login' )
			),
			'break-1'           => array(
				'type'            => 'break'
			),
			'logged_in_title'  => array(
				'label'           => __( 'Logged-in title', 'songer-sidebar-login' ),
				'default'         => __( 'Welcome %username%', 'songer-sidebar-login' ),
				'type'            => 'text'
			),
			'logged_in_links'  => array(
				'label'           => __( 'Links', 'songer-sidebar-login' ) . ' (<code>' . __( 'Text | HREF | Capability', 'songer-sidebar-login' ) . '</code>)',
				'description'     => sprintf( __( '<a href="%s">Capability</a> (optional) refers to the type of user who can view the link.', 'songer-sidebar-login' ), 'http://codex.wordpress.org/Roles_and_Capabilities' ),
				'default'         => __( "Dashboard | %admin_url%\nProfile | %admin_url%/profile.php\nLogout | %logout_url%", 'songer-sidebar-login' ),
				'type'            => 'textarea'
			),
			'show_avatar'  => array(
				'label'           => __( 'Show logged-in user avatar', 'songer-sidebar-login' ),
				'default'         => 1,
				'type'            => 'checkbox'
			),
			'logout_redirect_url'  => array(
				'label'           => __( 'Logout Redirect URL', 'songer-sidebar-login' ),
				'default'         => '',
				'type'            => 'text',
				'placeholder'     => __( 'Current page URL', 'songer-sidebar-login' )
			)
    ...
    public function replace_tags( $text ) {
	    if ( $this->user ) {
		    $text = str_replace(
		    	...
		    	array(
		    		...
		    		get_avatar( $this->user->ID, apply_filters( 'Songer_Sidebar_Login_Widget_avatar_size', 38 ) )
		    	),
		    	$text
		    );
	    ...
	    $text = str_replace(
	    	...
	    	array( untrailingslashit( admin_url() ), apply_filters( 'Songer_Sidebar_Login_Widget_logout_redirect', $logout_redirect ) ),
	    	$text
	    );
    ...
    public function show_links( $show = 'logged_in', $links = array() ) {
	    do_action( 'Songer_Sidebar_Login_Widget_before_' . $show . '_links' );
	    ...
	    if ( $show == 'logged_out' ) {
		    if ( get_option('users_can_register') && ! empty( $this->instance['show_register_link'] ) && $this->instance['show_register_link'] == 1 ) {
		    	...
		    	if ( ! is_multisite() ) {
		    		...
		    		$links['register'] = array(
		    			'text' => __( 'Register', 'songer-sidebar-login' ),
		    			'href' => apply_filters( 'Songer_Sidebar_Login_Widget_register_url', site_url( 'wp-login.php?action=register', 'login' ) )
		    		);
					...
					$links['register'] = array(
		    			'text' => __( 'Register', 'songer-sidebar-login' ),
		    			'href' => apply_filters( 'Songer_Sidebar_Login_Widget_register_url', site_url('wp-signup.php', 'login') )
		    		);
		    ...
		    if ( ! empty( $this->instance['show_lost_password_link'] ) && $this->instance['show_lost_password_link'] == 1 ) {
		    	...
		    	$links['lost_password'] = array(
	    			'text' => __( 'Lost Password', 'songer-sidebar-login' ),
	    			'href' => apply_filters( 'Songer_Sidebar_Login_Widget_lost_password_url', wp_lostpassword_url() )
	    		);

		    }
	    }

		$links = apply_filters( 'Songer_Sidebar_Login_Widget_' . $show . '_links', $links );
		...
		do_action( 'Songer_Sidebar_Login_Widget_after_' . $show . '_links' );
    }
    ...
    public function widget( $args, $instance ) {
		...
		if ( ! apply_filters( 'Songer_Sidebar_Login_Widget_display', true ) ) {
			return;
		}
		...
		$defaults = array(
			'logged_in_title'  => ! empty( $instance['logged_in_title'] ) ? $instance['logged_in_title'] : __( 'Welcome %username%', 'songer-sidebar-login' ),
			'logged_out_title' => ! empty( $instance['logged_out_title'] ) ? $instance['logged_out_title'] : __( 'Login', 'songer-sidebar-login' ),
		...
		do_action( 'Songer_Sidebar_Login_Widget_start' );

		// Logged in user
		if ( is_user_logged_in() ) {

			$logged_in_title = $this->replace_tags( apply_filters( 'Songer_Sidebar_Login_Widget_logged_in_title', $logged_in_title ) );

			if ( $logged_in_title ) {
				echo $before_title . $logged_in_title . $after_title;
			}

			do_action( 'Songer_Sidebar_Login_Widget_logged_in_content_start' );

			if ( $show_avatar == 1 ) {
				echo '<div class="avatar_container">' . get_avatar( $this->user->ID, apply_filters( 'Songer_Sidebar_Login_Widget_avatar_size', 38 ) ) . '</div>';
			}

			$this->show_links( 'logged_in', $logged_in_links );

			do_action( 'Songer_Sidebar_Login_Widget_logged_in_content_end' );

		// Logged out user
		} else {

			$logged_out_title = $this->replace_tags( apply_filters( 'Songer_Sidebar_Login_Widget_logged_out_title', $logged_out_title ) );

			if ( $logged_out_title ) {
				echo $before_title . $logged_out_title . $after_title;
			}

			do_action( 'Songer_Sidebar_Login_Widget_logged_out_content_start' );
			...
			$login_form_args = apply_filters( 'Songer_Sidebar_Login_Widget_form_args', array(
		        'echo' 				=> false,
		        'redirect' 			=> esc_url( apply_filters( 'Songer_Sidebar_Login_Widget_login_redirect', $redirect ) ),
		        'label_username' 	=> __( 'Username', 'songer-sidebar-login' ),
		        'label_password' 	=> __( 'Password', 'songer-sidebar-login' ),
		        'label_remember' 	=> __( 'Remember Me', 'songer-sidebar-login' ),
		        'label_log_in' 		=> __( 'Login &rarr;', 'songer-sidebar-login' ),
		        'remember' 			=> $show_rememberme,
		        'value_remember' 	=> true
		    ) );

			echo apply_filters( 'Songer_Sidebar_Login_Widget_wp_login_form', wp_login_form( $login_form_args ), $login_form_args );

			$this->show_links( 'logged_out', $logged_out_links );

			do_action( 'Songer_Sidebar_Login_Widget_logged_out_content_end' );
		}

		do_action( 'Songer_Sidebar_Login_Widget_end' );
...
register_widget( 'Songer_Sidebar_Login_Widget' );
