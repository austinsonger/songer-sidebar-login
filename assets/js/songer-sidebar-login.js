jQuery(function(){
	...
	jQuery('.widget_wp_sidebarlogin form').submit(function(){
	    ...
	    jQuery('.'+songer_sidebar_login_params.error_class).remove();
	    ...
	    if ( ! user_login ) {
	    	$thisform.prepend('<p class="' + songer_sidebar_login_params.error_class + '">' + songer_sidebar_login_params.i18n_username_required + '</p>');
	    	return false;
	    }
	    if ( ! user_password ) {
			$thisform.prepend('<p class="' + songer_sidebar_login_params.error_class + '">' + songer_sidebar_login_params.i18n_password_required + '</p>');
	    	return false;
	    }

		// Check for SSL/FORCE SSL LOGIN
		if ( songer_sidebar_login_params.force_ssl_admin == 1 && songer_sidebar_login_params.is_ssl == 0 )
			return true;
	    ...
	    var data = {
			action: 		'songer_sidebar_login_process',
		...
		// Ajax action
		jQuery.ajax({
			url: songer_sidebar_login_params.ajax_url,
			data: data,
			type: 'POST',
			success: function( response ) {
				...
				if ( result.success == 1 ) {
					...
					$thisform.prepend('<p class="' + songer_sidebar_login_params.error_class + '">' + result.error + '</p>');
					$thisform.unblock();
				}
			}
